...
class GhanaRouteOptimizer:
    ...
    def load_data(self):
        """Load and preprocess transport data from Django ORM, filtered by city if provided"""
        print("Loading transport data for optimization from database...")
        if self.city:
            self.data['routes'] = pd.DataFrame(list(Route.objects.filter(source=self.city).values()))
            self.data['stops'] = pd.DataFrame(list(Station.objects.filter(gtfs_source=self.city).values()))
            ...
            self.data['stops'] = pd.DataFrame(list(Station.objects.all().values()))
    ...
    def prepare_route_features(self):
        ...
        routes = self.data['routes'] if 'routes' in self.data else pd.DataFrame()
        stops = self.data['stops'] if 'stops' in self.data else pd.DataFrame()
        stop_times = self.data['stop_times'] if 'stop_times' in self.data else pd.DataFrame()
        trips = self.data['trips'] if 'trips' in self.data else pd.DataFrame()
        if routes.empty or stops.empty or stop_times.empty or trips.empty:
            print("❌ Not enough data to prepare route features.")
            self.route_features_df = pd.DataFrame()
            return
    ...
    def train_efficiency_model(self):
        ...
        feature_cols = ['route_type', 'n_trips', 'n_stops', 
    ...
    def generate_optimization_report(self):
        ...
        print("3. Optimize route coverage gaps")
        print("4. Implement real-time optimization")
